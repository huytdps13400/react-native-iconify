...
function loadNativeModule(): NativeCacheModule {
  ...
  // Try Expo modules first (for development builds)
  try {
    // eslint-disable-next-line @typescript-eslint/no-var-requires
    const ExpoModulesCore = require('expo-modules-core');
    if (ExpoModulesCore?.requireNativeModule) {
      module = ExpoModulesCore.requireNativeModule('TurboCache') as NativeCacheModule;
      if (module) {
        console.log('[TurboCache] ✅ Loaded via Expo modules (SDWebImage/Glide)');
        return module;
      }
    }
  } catch (expoError) {
    // Expo modules not available, try React Native NativeModules
  }

  // Try React Native NativeModules (for CLI projects)
  try {
    module = NativeModules.TurboCache as NativeCacheModule;
    if (module) {
      console.log('[TurboCache] ✅ Loaded via React Native NativeModules (SDWebImage/Glide)');
      return module;
    }
  } catch (rnError) {
  ...
  try {
    ...
    if (TurboModuleRegistry) {
      ...
      if (module) {
        console.log('[TurboCache] ✅ Loaded via TurboModuleRegistry (SDWebImage/Glide)');
        return module;
      }
    }
  } catch (turboError) {
...
export class NativeDiskCache<T = unknown> {
  ...
  async get(key: string): Promise<T | null> {
    try {
      const value = await this.nativeModule.get(key);
  ...
  async set(key: string, data: T, ttl?: number): Promise<void> {
    try {
      ...
      await this.nativeModule.set(key, value, ttl);
    } catch (error) {
