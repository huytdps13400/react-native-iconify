...
// Initialize cache instance for icon data
// Using native-only cache (SDWebImage/Glide) to avoid Hermes errors
const cache = createCache<IconData>({
  maxSize: 1000,
  ...
  useEffect(() => {
    ...
    async function loadIconData() {
      try {
        ...
        // Try native cache first (SDWebImage/Glide handles memory → disk)
        ...
        await cache.set(cacheKey, fetchedIconData);
